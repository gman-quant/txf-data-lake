...
#### 🟢 機構級智慧轉倉 (Smart Rollover)
結算日（第三個星期三）往往伴隨首月合約 (R1) 失去流動性或出現壓低/拉高結算等價格失真現象。此功能將完美濾除結算雜訊，還原市場真實意圖：
```bash
python view_chart.py --live --smart-rollover
```
- **自動迴避失真**：精準偵測結算日，將「結算日前一晚夜盤」至「結算日當天日盤」的主圖表與技術指標，自動無縫抽換為次月合約 (R2) 報價。
- **正價差/逆價差照妖鏡**：結合副圖表的 **期現價差 (Basis)** 與 **跨月轉倉價差 (R2-R1)**，一眼看穿主力在次月合約的真實多空情緒。
- **無縫接軌**：結算日當晚的夜盤（即新合約首日）將自動切回 R1，搭配即時 Kafka 串流，達成最純粹的機構級看盤體驗。
