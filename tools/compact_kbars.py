...
(⚠ 這句在 2026-08-24 之前對**寫入端是假的** —— main_etl 用自己的 `if tf=='1d'`
 分流,翻表後會照舊寫日檔到沒人讀的位置。該日已修:寫入統一走 `lake_writer.save_kbars`
 = 問 `kbar_paths` 拿容器 + 依 `layout_of` 決定合併;守衛 tests/test_lake_writer.py
 真的以 monthly 佈局跑過寫入端再用讀取端讀回。)
這支負責把既有的日檔併成月檔。
...
~~`main_etl` 的 Case A/B~~(2026-08-24 已先行消滅:併入 `lake_writer.save_kbars`,
佈局分流不存在了;**身分鍵的分岔仍在**,住 `lake_writer.DEDUP_KEY` —— 那是「1d 是
盤段棒、ts 不穩定」的語意差異,不隨佈局統一而消失)、`historical.py` 的
`if base_tf == "1d"`、以及上述身分鍵差異 —— 這些分岔多數來自 1d 的佈局與眾不同。
