...
- **2026-08-15 起 kbar 一律 12 欄**:`true_pt_sum`(∫P dt,棒邊界切片)+ `dur_s`(恆=桶名目長)——TWMA 的 tick 級可加量(語意正典=platform `wiki/MA-Semantics.md` §6)。全史已回填+例外已修復(嫁接→裁決→重建,`tools/backfill_pt_sum.py` + `tools/repair_pt_exceptions.py`);**唯一殘留 pt=null = TXFR2 2025-01-06**(重抓實測 Shioaji 只回日盤 —— 該日週五夜盤資料 API 端已滅失,舊 kbars 的夜盤棒是唯一倖存副本,**別再試圖修**)。⚠ resampler 的兩處 sort 帶 `maintain_order=True` 是**契約不是裝飾**:平手序=raw 檔到達序;拿掉它,特定 chunk 型態的檔重算就會換序(2026-08-15 實證 13 天中招)。備份:`kbars_backup_pre_ptsum/`(回填前)、`repair_backup_20260815/`(修復前)。
